...
author: Abdox
description: |
  ULTRA-ADVANCED Saga Guard - Saga/choreography pattern validation - compensating transactions, step ordering, orchestration
  ...
  TRIGGER PHRASES: "saga pattern, compensating transactions, choreography"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Saga Guard - ULTRA-ADVANCED v1.0

## Overview

Saga/choreography pattern validation - compensating transactions, step ordering, orchestration. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Missing compensating action | Saga step without rollback handler | critical |
| Step order wrong | Wrong execution sequence in saga | high |
| Compensating not idempotent | Compensation handler not idempotent | critical |
| Timeout missing on step | No timeout configured for saga step | high |
| Orchestrator bottleneck | Central orchestrator creates coupling | medium |
