...
author: Abdox
description: |
  ULTRA-ADVANCED Retry Guard - Retry pattern validation - exponential backoff, jitter, max retries, circuit breaker integration
  ...
  TRIGGER PHRASES: "retry pattern, exponential backoff, retry logic"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Retry Guard - ULTRA-ADVANCED v1.0

## Overview

Retry pattern validation - exponential backoff, jitter, max retries, circuit breaker integration. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Infinite retries | No maximum retry limit set | high |
| No backoff | Immediate retry without delay | medium |
| No jitter | Thundering herd risk on retry | medium |
| No circuit breaker | Retry storm on down service | high |
| Retry on non-idempotent | Duplicate side effects from retry | critical |
