---
name: graceful-shutdown-guard
version: 1.0.0
author: Abdox
description: |
  ULTRA-ADVANCED Graceful Shutdown Guard - Graceful shutdown validation - signal handling, drain, cleanup, health check on stop
  ...
  TRIGGER PHRASES: "graceful shutdown, signal handling, clean shutdown"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Graceful Shutdown Guard - ULTRA-ADVANCED v1.0

## Overview

Graceful shutdown validation - signal handling, drain, cleanup, health check on stop. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Missing signal handler | SIGTERM/SIGINT not caught | critical |
| No connection drain | Active requests dropped on shutdown | high |
| No resource cleanup | Files or connections not released | high |
| Shutdown timeout missing | Forced kill without drain period | medium |
| No health check on stop | Load balancer not notified to drain | medium |
