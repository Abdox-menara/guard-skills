...
author: Abdox
description: |
  ULTRA-ADVANCED Jwt Guard - JWT implementation validation - token handling, algorithms, security patterns
  ...
  TRIGGER PHRASES: "jwt check, token validation, json web token"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Jwt Guard - ULTRA-ADVANCED v1.0

## Overview

JWT implementation validation - token handling, algorithms, security patterns. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Hardcoded JWT secret | JWT secret found in source code | critical |
| No signature verification | JWT accepted without signature check | critical |
| Weak algorithm | alg: none or HS256 with RSA public key | critical |
| Missing expiry check | Token expiration not validated | high |
| Sensitive data in payload | PII or secrets in JWT payload | high |
