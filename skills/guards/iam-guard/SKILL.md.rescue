...
author: Abdox
description: |
  ULTRA-ADVANCED Iam Guard - IAM policy validation - least privilege, resource constraints, condition blocks
  ...
  TRIGGER PHRASES: "iam check, access management, least privilege"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Iam Guard - ULTRA-ADVANCED v1.0

## Overview

IAM policy validation - least privilege, resource constraints, condition blocks. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Wildcard IAM action | Action set to * wildcard | critical |
| Wildcard IAM resource | Resource set to * wildcard | critical |
| Overly permissive role | Too many privileges assigned to role | high |
| Missing condition block | No condition constraints on policy | medium |
| Cross-account trust broad | Trust policy without conditions | high |
