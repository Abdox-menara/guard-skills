...
author: Abdox
description: |
  ULTRA-ADVANCED Xss Guard - XSS prevention validation - output encoding, CSP headers, dangerous API detection
  ...
  TRIGGER PHRASES: "xss check, cross site scripting, output encoding"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Xss Guard - ULTRA-ADVANCED v1.0

## Overview

XSS prevention validation - output encoding, CSP headers, dangerous API detection. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Unescaped user input | Rendering raw user input to DOM | critical |
| innerHTML with user data | innerHTML assignment with variables | critical |
| document.write usage | document.write with variable content | high |
| Dangerous HTML attributes | on* event handlers with user input | critical |
| Template injection | Unsafe server-side template rendering | high |
