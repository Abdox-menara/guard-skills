...
author: Abdox
description: |
  ULTRA-ADVANCED Injection Guard - Injection prevention validation - OS command, LDAP, XPath, NoSQL, template injection
  ...
  TRIGGER PHRASES: "injection check, command injection, nosql injection"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Injection Guard - ULTRA-ADVANCED v1.0

## Overview

Injection prevention validation - OS command, LDAP, XPath, NoSQL, template injection. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| OS command injection | subprocess/shell with user input | critical |
| eval/exec usage | Dynamic code execution with variables | critical |
| LDAP injection | LDAP query built from user input | high |
| XPath injection | XPath expression from user input | high |
| NoSQL injection | NoSQL query from unsanitized input | high |
