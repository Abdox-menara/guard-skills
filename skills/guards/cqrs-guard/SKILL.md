...
author: Abdox
description: |
  ULTRA-ADVANCED Cqrs Guard - CQRS pattern validation - command/query separation, read/write models, separate stores
  ...
  TRIGGER PHRASES: "cqrs, command query segregation, read model"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Cqrs Guard - ULTRA-ADVANCED v1.0

## Overview

CQRS pattern validation - command/query separation, read/write models, separate stores. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Command returning data | Command handler returns query results | critical |
| Read model stale | No refresh strategy for read models | high |
| Command validation leak | Validation logic mixed with handler | medium |
| Query affecting state | Query handler has side effects | critical |
| Shared model violation | Read and write share same model | high |
