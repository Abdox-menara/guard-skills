---
name: event-sourcing-guard
version: 1.0.0
author: Abdox
description: |
  ULTRA-ADVANCED Event Sourcing Guard - Event sourcing pattern validation - event streams, aggregates, snapshots, replay
  ...
  TRIGGER PHRASES: "event sourcing, event store, aggregate events"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Event Sourcing Guard - ULTRA-ADVANCED v1.0

## Overview

Event sourcing pattern validation - event streams, aggregates, snapshots, replay. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| Event mutation | Events modified after being appended | critical |
| Missing event type | Event without type discriminator field | high |
| Event version missing | No versioning schema on events | high |
| Aggregate missing stream | Aggregate without event stream | critical |
| Snapshot strategy missing | No snapshot for long event streams | medium |
