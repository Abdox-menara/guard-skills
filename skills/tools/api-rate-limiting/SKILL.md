...
author: Abdox
description: |
  ULTRA-ADVANCED Api Rate Limiting - API rate limiting - algorithm selection, headers, throttling, quota management
  ...
  TRIGGER PHRASES: "rate limiting, api throttling, quota management"
  
  ENVIRONMENT: Works with any codebase, any language, any framework.

---

# Api Rate Limiting - ULTRA-ADVANCED v1.0

## Overview

API rate limiting - algorithm selection, headers, throttling, quota management. This skill scans codebases for violations, scores compliance, and generates actionable recommendations.

## Detection Patterns

| Pattern | Description | Severity |
|---|---|---|
| No rate limiting | API endpoint without rate limit | critical |
| Missing rate limit headers | Retry-After or RateLimit not sent | medium |
| Global limit only | No per-user or per-IP limits | high |
| Window too large | Rate limit window > 1 hour | medium |
| No burst allowance | No short burst capacity | medium |
