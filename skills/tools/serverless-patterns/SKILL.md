...
author: Abdox
description: |
  ULTRA-ADVANCED Serverless Patterns - Serverless patterns - function design, cold start, event sources, error handling
  CAPABILITIES:
  - 8 anti-pattern detection patterns with severity scoring
  - Compliance scoring (A-F grade) with weighted severity model
  - Standards-based detection (AWS Lambda best practices; WA Serverless)
  - Automated fix suggestion generation
  - Directory scanning with aggregated reporting
  TRIGGER PHRASES: "serverless patterns, lambda, functions, faas"
  STANDARDS: AWS Lambda best practices; WA Serverless
  ENVIRONMENT: Works with any codebase, any language, any framework.
  SECURITY: Read-only analysis.
---

# Serverless Patterns - ULTRA-ADVANCED v2.0

## Overview

Serverless patterns - function design, cold start, event sources, error handling. This guard scans codebases for violations, scores compliance,
