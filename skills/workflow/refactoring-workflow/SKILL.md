...
author: Abdox
description: |
  ULTRA-ADVANCED Refactoring Workflow - Refactoring workflow - code smells, safe steps, test coverage, incremental improvement
  CAPABILITIES:
  - 8 anti-pattern detection patterns with severity scoring
  - Compliance scoring (A-F grade) with weighted severity model
  - Standards-based detection (Martin Fowler Refactoring; Legacy Code)
  - Automated fix suggestion generation
  - Directory scanning with aggregated reporting
  TRIGGER PHRASES: "refactoring workflow, code smell removal, code improvement"
  STANDARDS: Martin Fowler Refactoring; Legacy Code
  ENVIRONMENT: Works with any codebase, any language, any framework.
  SECURITY: Read-only analysis.
---

# Refactoring Workflow - ULTRA-ADVANCED v2.0

## Overview

Refactoring workflow - code smells, safe steps, test coverage, incremental improvement. This guard scans codebases for violations, scores compliance,
